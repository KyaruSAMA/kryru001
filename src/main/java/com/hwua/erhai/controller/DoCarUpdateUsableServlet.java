...
public class DoCarUpdateUsableServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (newCar==null){
            ...
            response.getWriter().write("服务器内部错误，更新汽车上下架失败");

        }
        response.setStatus(HttpServletResponse.SC_OK);
        DoCarUpdateUsableResponse usableResponse =new DoCarUpdateUsableResponse(
                HttpServletResponse.SC_OK,"更新汽车上下架状态成功",
                car.getId(),car.getUsable()
        );
        response.getWriter().write(JSONObject.toJSONString(usableResponse));
    }
    }
